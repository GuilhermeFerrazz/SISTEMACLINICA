...
const Products = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Converte cost_price para número (ou remove se vazio)
      if (payload.cost_price === '' || payload.cost_price === null) {
        delete payload.cost_price;
      } else {
        payload.cost_price = parseFloat(payload.cost_price) || 0;
      }
      // fill_date é opcional: string vazia falha na validação de formato (AAAA-MM-DD) no backend
      if (!payload.fill_date) {
        delete payload.fill_date;
      }
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium text-foreground">{product.quantity}{product.unit && product.unit !== 'un' ? ` ${product.unit}` : ''}</p>
                      </div>
                      <div>
