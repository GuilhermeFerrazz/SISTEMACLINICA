...
// Codifica mensagem para URL do WhatsApp usando UTF-8 percent-encoding padrão.
// Esse é o formato oficial documentado pela Meta para a Click-to-Chat API.
// Emojis viram sequências %F0%9F%91%8B que o WhatsApp Web decodifica corretamente.
// (A versão antiga "deixava Unicode cru" e quebrava emojis após o JS do WhatsApp Web rodar.)
const encodeWhatsApp = (msg) => encodeURIComponent(msg || '');

// Decodifica mensagens salvas com prefixo B64: (proteção de emojis no banco)
const decodeB64 = (str) => {
  if (!str || !str.startsWith('B64:')) return str;
  try {
    const encodedPart = str.slice(4);
...
const Agenda = () => {
  ...
  const handleTemplateSelect = async (templateId) => {
    ...
    // Decodifica B64 se necessário e remove variation selectors soltos que quebram WhatsApp Web
    let msg = decodeB64(template.message);
    msg = msg.replace(/(?<=\s)[\ufe0e\ufe0f]/g, '').replace(/^[\ufe0e\ufe0f]/g, '');
    ...
    // Gera link de confirmação/cancelamento para o paciente quando o template
    // for de confirmação OU contiver explicitamente {link_confirmacao}.
    const hasLinkVar = msg.includes('{link_confirmacao}');
    const isConfirmationType = template.type === 'appointment_confirmation';
    if (hasLinkVar || isConfirmationType) {
      try {
        const { data } = await axios.post(
          `${API}/appointments/${whatsappAppointment.id}/confirmation-link`,
          {},
          { withCredentials: true }
        );
        const link = data?.link || '';
        if (hasLinkVar) {
          // Usuário posicionou o placeholder no template — apenas substitui.
          msg = msg.replace(/\{link_confirmacao\}/g, link);
        } else if (link) {
          // Template não tem o placeholder — adiciona apenas a URL pura no final,
          // sem texto extra, para preservar a formatação original do template.
          // (Para customizar a posição do link, adicione {link_confirmacao} no template.)
          msg = msg.trimEnd() + `\n\n${link}`;
        }
      } catch (err) {
        console.error('Erro ao gerar link de confirmação:', err);
        toast.error('Não foi possível gerar o link de confirmação. A mensagem será enviada sem o link.');
        // Remove o placeholder para não enviar o texto cru "{link_confirmacao}"
        msg = msg.replace(/\{link_confirmacao\}/g, '').replace(/\n{3,}/g, '\n\n').trim();
      }
    }
  ...
  const handleSendWhatsApp = () => {
    ...
    // Monta a URL do WhatsApp usando o endpoint oficial Click-to-Chat da Meta.
    // api.whatsapp.com/send é mais robusto que wa.me (este último faz redirect
    // que em alguns dispositivos reinterpreta bytes UTF-8 como Latin-1 e quebra emojis).
    const url = `https://api.whatsapp.com/send?phone=${phone}&text=${encodeWhatsApp(previewMessage)}`;
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {apt.patient_confirmed && (
                              <div className={`inline-flex items-center gap-1 mt-2 px-2 py-0.5 rounded-full text-xs font-medium ${
                                apt.patient_confirmed === 'confirmed'
                                  ? 'bg-green-100 text-green-700'
                                  : 'bg-red-100 text-red-700'
                              }`}>
                                {apt.patient_confirmed === 'confirmed' ? '✅ Confirmado pelo paciente' : '❌ Cancelado pelo paciente'}
                              </div>
                            )}
                            {apt.notes && (
                            ...
                            {apt.status !== 'completed' && apt.status !== 'cancelled' && (
                              <>
                                ...
                                <Button
                                  ...
                                  onClick={() => navigate(`/atendimento/agenda/${apt.id}`)}
                                  data-testid={`start-consultation-button-${apt.id}`}
                                >
            ...
            <div className="space-y-4 mt-2">
              ...
              {previewMessage && (
                <div className="min-w-0">
                  ...
                  <div className="p-4 bg-[#e5ddd5] rounded-lg overflow-hidden">
                    <div className="bg-[#dcf8c6] rounded-lg p-3 max-w-[85%] ml-auto shadow-sm overflow-hidden">
                      <p
                        className="text-sm whitespace-pre-wrap break-all text-gray-800"
                        style={{ fontSize: '13px', overflowWrap: 'anywhere', wordBreak: 'break-word' }}
                      >
                        {previewMessage}
                      </p>
